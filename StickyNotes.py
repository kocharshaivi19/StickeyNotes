...
class mainmenu:
	def __init__(self,root):
		...
		for l in self._listi:
		    Sourcemenu.add_command(label=l, command=lambda arg0=l: self.list_priority(arg0))
	...
	#list the notePad according to the priority details for a particular day
	#append the Date, Time, fileName in a csv file with its priority
	def list_priority(self, arg0): 
	...
	#display the entry box to read the user specified date
	def display(self):
	...
	#list down the data in a Messagebox for a specified date format &Y-%m-%d %H:%M:%S
	def GetData(self):
		fieldnames = ['time1','filename', 'priority']
